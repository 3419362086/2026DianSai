...
#define EMM_V5_UART_TIMEOUT_MS  1000U

/* Blocking send keeps command buffers valid; do not call from an ISR. */

static void Emm_V5_Transmit(UART_HandleTypeDef *huart, uint8_t *cmd, uint16_t len)
{
  if (huart == NULL)
  {
    return;
  ...
  (void)HAL_UART_Transmit(huart, cmd, len, EMM_V5_UART_TIMEOUT_MS);
}
