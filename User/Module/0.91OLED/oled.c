 ...
 * @param x1  Image display end position x-axis 1 - 128
...
*/
void OLED_ShowPic(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t BMP[], unsigned char reverse_flag)
{
	uint16_t i = 0;
	uint8_t x, y;
    if(x0 == 0 && y0 == 0 && x1 == 128 && y1 == 4)
    {
        if(reverse_flag == 0)
        {
            HAL_I2C_Mem_Write(&hi2c2, 0x78, 0x40, I2C_MEMADD_SIZE_8BIT, BMP, 128 * 4, 0x100);
        }
        else
        {
...
void OLED_ShowChar2(uint8_t x, uint8_t y, uint8_t ch, uint8_t fontsize, unsigned char reverse_flag)
{
	uint8_t c = 0, i = 0;
	c = ch - ' ';
...
//	}
    ...
    HAL_I2C_Mem_Write(&hi2c2, 0x78, 0x40, I2C_MEMADD_SIZE_8BIT, temp_buffer, 512, 0x100);
}
/**
...
void OLED_Init(void)
{
	...
	OLED_Set_Position(0, 0);
//    
//    OLED_Write_cmd(0xB0); // 设置 OLED 页地址为 0
//	
//	OLED_Write_cmd(0x00); // 设置 OLED 列地址的低 4 位为 0
//	OLED_Write_cmd(0x10); // 设置 OLED 列地址的高 4 位为 0
}

void Oled_Show_Chinese_Char(unsigned char x, unsigned char y, unsigned char font_size, unsigned char target_char[2], unsigned char reverse_flag)
{
	/* 判断字符大小 */
	switch(font_size)
	{
		case 16:
			...
			{
				if(CHINESE_16x16[index].index[0] == target_char[0] && CHINESE_16x16[index].index[1] == target_char[1])
				{
                    uint8_t t, adder = 0;
                    if(reverse_flag == 0)
                    {
                        ...
                        for (t = 0; t < 16; t++)
                        {
                            OLED_Write_data(CHINESE_16x16[index].data[t]);
                            adder += 1;
                        }
                        OLED_Set_Position(x, y + 1);
                        for (t = 16; t < 32; t++)
